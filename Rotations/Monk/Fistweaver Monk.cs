...
namespace HyperElk.Core
{
    public class FistweaverMonk : CombatRoutine
    {
        ...
        private static bool CantDetoxUnit(string debuff, string unit)
        {
        ...
        public override void Initialize()
        {
            ...
            CombatRoutine.AddSpell(BonedustBrew, 325216);
        ...
        public override void Pulse()
        {
            ...
            if (IsAutoDetox)
            {
                ...
                if (API.CanCast(Detox) && NotChanneling && !API.PlayerIsMounted)
                {
                    for (int i = 0; i < DetoxList.Length; i++)
                        for (int j = 0; j < units.Length; j++)
                        {
                            if (IsAutoDetox && !DispelWatch.IsRunning && CantDetoxUnit("Frozen Binds", units[j]))
                            {
                                DispelWatch.Start();
                                return;
                            }
                            if (CanDetoxTarget("Frozen Binds") && DispelWatch.ElapsedMilliseconds >= 3000)
                            {
                                API.CastSpell(Detox);
                                DispelWatch.Reset();
                                return;
                            ...
                            if (CantDetoxUnit("Frozen Binds", units[j]) && IsAutoDetox && !API.SpellISOnCooldown(Detox) && API.UnitRange(units[j]) <= 40)
                            {
                                API.CastSpell(PlayerTargetArray[j]);
                                return;
                            }
                            if (CantDetoxUnit(DetoxList[i], units[j]) && IsAutoDetox && !API.SpellISOnCooldown(Detox) && API.UnitRange(units[j]) <= 40)
                            {
                                API.CastSpell(PlayerTargetArray[j]);
                                return;
                            }
            ...
            if (!RangeHeal && Fistweaving && !ChannelSoothingMist)
            {
                ...
                if (API.CanCast(BonedustBrew) && PlayerCovenantSettings == "Necrolord")
                {
