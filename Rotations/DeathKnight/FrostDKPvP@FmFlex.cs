...
namespace HyperElk.Core
{
    public class LevelingDK : CombatRoutine
    {
        ...
        //CBProperties
        private int IceboundFortitudeLifePercent => API.getFromArray(percentListProp,CombatRoutine.GetPropertyInt(IceboundFortitude));
        private int AntiMagicShellLifePercent => API.getFromArray(percentListProp,CombatRoutine.GetPropertyInt(AntiMagicShell));
        private int LichborneLifePercent => API.getFromArray(percentListProp,CombatRoutine.GetPropertyInt(Lichborne));
        private int DeathStrikeProcLifePercent => API.getFromArray(percentListProp,CombatRoutine.GetPropertyInt(DeathStrike + "PROC"));
        private int DeathStrikeLifePercent => API.getFromArray(percentListProp,CombatRoutine.GetPropertyInt(DeathStrike));
      ...
      cooldowns -> add_action( this, "Pillar of Frost", "if=talent.obliteration&(talent.gathering_storm.enabled&buff.remorseless_winter.up|!talent.gathering_storm.enabled)" );
            ...
            */
                ...
                if (PlayerLevel >= 30 && IsAOE && API.CanCast(ChillStreak, true, true)&& API.PlayerHasBuff(PillarofFrost) && API.TargetRange <= 40)
                {
