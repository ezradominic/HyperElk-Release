...
namespace HyperElk.Core
{
    public class BMHunter : CombatRoutine
    {
        ...
        {
            // API.WriteLog("Has Pet  " + API.PlayerHasPet + "HP " + API.PetHealthPercent);
        ...
        public override void CombatPulse()
        {
            if (API.PetHealthPercent >= 1 && !API.PlayerIsMounted && !API.PlayerIsCasting && !API.PlayerIsChanneling && !PlayerHasBuff(Aspect_of_the_Turtle) && !PlayerHasBuff(Feign_Death))
            {
                ...
                if (!IsAOE || API.TargetUnitInRangeCount < AOEUnitNumber)
                {
                    ...
                    if (API.CanCast(Death_Chakram) && API.PlayerFocus + RealFocusRegen * gcd / 100 < API.PlayerMaxFocus && PlayerCovenantSettings == "Necrolord" && (UseCovenant == "with Cooldowns" && IsCooldowns || UseCovenant == "always" || UseCovenant == "on AOE" && API.PlayerUnitInMeleeRangeCount >= AOEUnitNumber && IsAOE) && InRange)
                    {
                    ...
                    if (API.CanCast(Resonating_Arrow) && PlayerCovenantSettings == "Kyrian" && (PlayerHasBuff(Bestial_Wrath) || API.TargetTimeToDie < 1000) && (UseCovenant == "with Cooldowns" && IsCooldowns || UseCovenant == "always" || UseCovenant == "on AOE" && API.PlayerUnitInMeleeRangeCount >= AOEUnitNumber && IsAOE) && InRange)
                    {
                ...
                if (IsAOE && API.TargetUnitInRangeCount >= AOEUnitNumber)
                {
                    ...
                    if (API.CanCast(Death_Chakram) && API.PlayerFocus + RealFocusRegen * gcd / 100 + 3 * API.TargetUnitInRangeCount < API.PlayerMaxFocus && PlayerCovenantSettings == "Necrolord" && (UseCovenant == "with Cooldowns" && IsCooldowns || UseCovenant == "always" || UseCovenant == "on AOE" && API.PlayerUnitInMeleeRangeCount >= AOEUnitNumber && IsAOE) && InRange)
                    {
                        API.CastSpell(Death_Chakram);
                        return;
                    }
